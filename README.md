...
专用 AC/PY/PL/FC 字段优先显示，长表字段位于后面。两种方式请二选一；混合绑定会显示提示，闲置“值”的格式也不会影响专用度量。

## 1.8.3 正确性与可读性更新

- 横向图支持负实际值，空值不再画成极细柱；合计与明细使用同一实际值比例尺。
- 瀑布保留单侧缺失类别的已有值及完整合计。缺失侧用中性色、星号和说明标明“衔接合计的调整”，不当作真实业绩差异。
- 时间序列使用独立槽位区分同名成员；密集数值标签自动避让，完整数值仍可在工具提示读取。工具提示包含选定基准的绝对及相对差异。
- 多基准面板按选定主基准统一排序；自动模式优先 PY、PL、FC。
- 「场景 → 使用数据字段名称作为场景名称」可让图例使用度量名，例如“上周销售额”；手工指定的自定义名称仍优先。
- 「图表 → 跨类别汇总」支持自动、求和、不汇总。自动模式根据主值格式排除百分比；**均值、单价、去重门店数等必须手动选择“不汇总”**。该设置关闭合计、“其他”及求和瀑布，不会自动推断 DAX 的聚合语义。
- “其他”的附加工具提示度量不再求和，因为视觉对象无法取得这些度量在“其他”筛选上下文中的正确结果。

此版本仍要求传入适合目标图表粒度的数据。尤其非可加度量，不应依赖视觉对象把“类别×月份”等更细粒度结果重新求和；应在模型中按目标粒度计算。预测分段、层级下钻、业务分组小多图和可配置瀑布小计仍为后续功能。
